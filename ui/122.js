...
const friends_panel = new function() {
    ...
    this.init = () => {
        ...
        html.party_privacy = html.settings_party.querySelector(".party_privacy");
        html.party_leave = html.settings_party.querySelector(".party_leave");
        html.party_head = html.settings_party.querySelector(".party_head");
        ...
        on_own_data_changed.push((() => {
            let setting = html.allow_friend_requests.querySelector(".toggle");
            set_toggle(setting, global_self.friend_requests, true)
        }));
        ...
        global_on_ms_disconnected.push((() => {
            ...
            let setting = html.allow_friend_requests.querySelector(".toggle");
            setup_toggle(setting, global_self.friend_requests, ((toggle, bool) => {
                if (bool) {
    ...
    function update_settings_visibility() {
        if (tab_map.current_tab === "friends_panel_tab_party") {
            ...
            html.party_head.style.display = "flex"
        } else {
            html.party_privacy.style.display = "none";
            html.party_privacy.classList.add("disabled");
            html.party_leave.style.display = "none";
            html.party_leave.classList.add("disabled");
            html.party_head.style.display = "none"
        }
        if (tab_map.current_tab === "friends_panel_tab_list") {
    ...
    function render_party_list() {
        ...
        html.party_head.textContent = localize("party") + " ( " + party.length + " / 4 )";
        for (let i = start_index; i < start_index + MAX_ITEMS_PER_PAGE; i++) {
    ...
    function create_party_el(f) {
        ...
        let name = f.name;
        if (global_party.size > 1 && global_party.leader_id === f.user_id) {
            name += " (Leader)"
        }
        let desc = _createElement("div", "desc");
        desc.appendChild(_createElement("div", "name", name));
    ...
    function create_friend_icon(type, f) {
            ...
            if (f.category) {
                if (f.category === "othergame" || f.category === "online" || f.category === "offline" || f.category === "ingame") {
                    if (f.storefriend) {
                        ...
                        let icon = _createElement("div", ["avatar_icon", "master"]);
        ...
        if (type === "friend") {
            icon_cont.classList.add("has_status");
            icon_cont.appendChild(_createElement("div", "status_icon"))
        } else if (type === "party") {
            icon_cont.classList.add("has_status");
            icon_cont.appendChild(_createElement("div", ["status_icon", "ingame"]))
        }
        return icon_cont
    }
    ...
    function create_action_menu(el) {
        ...
        html.action_menu.style.top = rect.top + rect.height * .8 + "px";
        let options = [];
