...
const page_custom = new function() {
    ...
    let html = {
        ...
        settings_details_scroll: null,
        screen_actions: null,
        join_button: null,
        current_map_selection: null,
        current_mode_selection: null,
        team_head: {},
        team_counts: {},
        hovered_setting: null,
        descriptions: null,
        commands: null
    };
    let el_containers = {};
    ...
    let tab_map = {
        ...
        custom_screen_lobby_tab: {
            ...
            nav: null,
            cb: () => {
                refreshScrollbar(html.teams_scroll)
            }
        },
    ...
    this.init = () => {
        ...
        html.teams_scroll = html.lobby.querySelector(".teams_scroll.scroll-outer");
        html.spectators = html.root.querySelector(".team.spectators .list");
        html.settings_scroll = html.settings.querySelector(".settings_scroll.scroll-outer");
        html.settings_details_scroll = html.settings.querySelector(".details.scroll-outer");
        ...
        html.current_map_selection = html.root.querySelector(".map_selection .current_selection");
        ...
        html.descriptions = _id("custom_screen_settings").querySelector(".descriptions");
        ...
        let first_sub_menu_option = html.root.querySelector(".box_screen_menu_option");
        ...
        elements.mode_editing = _id("custom_game_setting_mode_editing");
        ...
        setup_toggle(elements.mode_editing, false, ((toggle, bool) => {
            this.user_update("mode_editing", bool ? 1 : 0)
        }));
        toggle_elements["mode_editing"] = elements.mode_editing;
        setup_toggle(elements.team_switching, false, ((toggle, bool) => {
            this.user_update("team_switching", bool ? 1 : 0)
        }));
        toggle_elements["team_switching"] = elements.team_switching;
        setup_toggle(elements.auto_balance, false, ((toggle, bool) => {
            this.user_update("auto_balance", bool ? 1 : 0)
        }));
        toggle_elements["auto_balance"] = elements.auto_balance;
        Lobby.add_setting_updated_listener(((setting, value) => {
            ...
            } else if (setting in toggle_elements) {
                set_toggle(toggle_elements[setting], value, true)
        ...
        Lobby.add_join_listener((() => {
            ...
            if (global_active_view === "menu" && global_menu_page === "main_panel_gearstorm") {
                open_screen("custom")
            }
            if (GAME.active === GAME.ids.GEARSTORM) return;
        ...
        Lobby.add_host_change_listener(((became_host, lost_host) => {
            if (became_host) {
                ...
                _for_each_with_selector_in_parent(_id("main_menu"), ".custom_game_setting .toggle", (el => {
                    this.remove_lock(el, "host")
                }));
                _for_each_with_selector_in_parent(_id("main_menu"), ".custom_game_setting .btn-style-4", (el => {
                    this.remove_lock(el, "host")
                }));
                this.remove_lock(elements.commands, "host");
                _id("custom_game_password_set").querySelector(".edit_password").style.display = "flex";
        ...
        GAME.add_activate_callback((game_id => {
            ...
            if (game_id === GAME.ids.ROGUE) {
                ...
                elements.team_switching.closest(".custom_game_setting").classList.remove("hidden");
                el_containers.name.classList.remove("hidden");
                el_containers.password.classList.remove("hidden")
            } else if (game_id === GAME.ids.GEARSTORM) {
                state.ui_max_teams = 1;
                ...
                elements.team_switching.closest(".custom_game_setting").classList.add("hidden");
        ...
        let settings = _id("custom_screen_settings").querySelectorAll(".custom_game_setting");
        for (let el of settings) {
            el.addEventListener("mouseenter", (() => {
                ...
                if (html.hovered_setting) {
                    html.hovered_setting.classList.remove("hovered")
                }
                html.hovered_setting = el;
                html.hovered_setting.classList.add("hovered");
                refreshScrollbar(html.settings_details_scroll);
                resetScrollbar(html.settings_details_scroll)
            }));
    ...
    this.user_update = (setting, value) => {
        if (html.hovered_setting) {
            html.hovered_setting.dispatchEvent(new Event("mouseenter"))
        }
        ...
        for (let setting of state.quick_info_settings) {
            ...
            if (setting === "private") {
                ...
                if (mode_data) {
                    ...
                    let mode_name = Lobby.get_setting("mode_name");
                    if (mode_name) value = mode_name
                }
    ...
    this.render_map_list = (map, idx) => {
        ...
        if (idx === 0) {
            ...
            html.current_map_selection.textContent = map_name;
            return
        }
        if (GAME.active === GAME.ids.ROGUE) {
    ...
    this.render_teams = teams => {
        ...
        if (html.teams_scroll) refreshScrollbar(html.teams_scroll)
    };
    ...
    this.open_map_selection = () => {
        ...
        if (GAME.active === GAME.ids.ROGUE || GAME.active === GAME.ids.GEARSTORM) {
    ...
    this.open_mode_selection = () => {
        if (!Lobby.state.host) return;
        Lobby.set_ai_mode("", null);
        mode_selection_modal.open(Lobby.get_setting("mode"), (async(mode, updated_ai_mode_prompt) => {
            if (mode) {
            ...
            if (updated_ai_mode_prompt) {
                try {
                    _id("ai_mode_error").style.display = "none";
                    setFullscreenSpinner(true);
                    console.log("Starting AI generation for description: ", updated_ai_mode_prompt);
                    const AI_TIMEOUT = 1e5;
                    const ai_mode_settings = await new Promise(((resolve, reject) => {
                        api_request("POST", "create_mode_ai", {
                            prompt: updated_ai_mode_prompt
                        }, (data => {
                    ...
                    console.log(ai_mode_settings);
                    Lobby.set_ai_mode(updated_ai_mode_prompt, ai_mode_settings)
                } catch (err) {
                    _id("ai_mode_error").style.display = "block";
                    console.error(err);
                    return false
                } finally {
    ...
    this.add_lock = (el, name) => {
        ...
        if (el.locks.length) {
            ...
            else if (el.classList.contains("toggle")) el.classList.add("locked");
            else if (el.classList.contains("btn-style-4")) el.classList.add("disabled")
        }
    };
    this.remove_lock = (el, name) => {
        ...
        if (el.locks.length) {
            ...
            else if (el.classList.contains("toggle")) el.classList.remove("locked");
            else if (el.classList.contains("btn-style-4")) el.classList.remove("disabled")
        }
    };
        ...
        if (team_count === 1 || team_count > 2 && team_size === 1) {
            ...
            for (let team_idx in html.team_head) {
                if (team_idx === "0") {
                    ...
                    html.team_head[team_idx].parentNode.style.display = "flex"
                } else {
                    html.team_head[team_idx].textContent = "";
                    html.team_head[team_idx].parentNode.style.display = "none"
                }
            }
            for (let team_idx in html.team_counts) {
...
function customPlayerContextMenu(e) {
    ...
    for (let o of this.contextOptions) {
        if ("buttons" in o && Array.isArray(o.buttons) && o.buttons.length) {
            if (o.buttons.includes(e.button)) {
                allow = true;
                break
            }
        }
    }
    if (e.button != 2 && !allow) return;
    e.preventDefault();
    let options = [];
    for (let o of this.contextOptions) {
        if (o.type == "stored") {
